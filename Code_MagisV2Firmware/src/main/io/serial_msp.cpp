...
void serialize8Debug ( uint8_t a ) {
  // Check if mspSerialPort is valid before writing
  if ( mspSerialPort != NULL ) {
    serialWrite ( mspSerialPort, a );
  } else if ( currentPort != NULL && currentPort->port != NULL ) {
    // Fallback: use currentPort if mspSerialPort is not initialized
    serialWrite ( currentPort->port, a );
  }
}
...
void mspInit ( serialConfig_t *serialConfig ) {
  ...
  // Find the first valid MSP port for debug output
  mspSerialPort = NULL;
  for ( uint8_t i = 0; i < MAX_MSP_PORT_COUNT; i++ ) {
    if ( mspPorts [ i ].port != NULL ) {
      mspSerialPort = mspPorts [ i ].port;
      break;
    }
  }

  // Fallback: if no MSP port was configured, try to open USART1 (ESP Wi‑Fi) for debug output
  if ( mspSerialPort == NULL ) {
    if ( serialIsPortAvailable ( SERIAL_PORT_USART1 ) ) {
      serialPort_t *fallback = openSerialPort ( SERIAL_PORT_USART1, FUNCTION_MSP, NULL, baudRates [ BAUD_115200 ], MODE_RXTX, SERIAL_NOT_INVERTED );
      if ( fallback != NULL ) {
        mspSerialPort = fallback;
      }
    }
  }
