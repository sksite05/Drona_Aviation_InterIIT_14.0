...
// extern int16_t NewSensorRange_L1;
...
void updateHeightEstimate(uint32_t currentTime)
{
    ...
    flow_rate_rps[1] = -(flowRate[1] + bodyRate[0]);
...
void calculateSensorFlow(uint32_t currentTime)
{
    ...
    uint8_t squal = paw3903_get_squal();
    uint8_t obs   = paw3903_get_observation();

    const uint8_t SQUAL_MIN = 20;   // tune later
    bool lowQuality = (squal < SQUAL_MIN);
    // Example: PAW3903 motion bit (bit7 of motion) already handled in driver;
    // you can also use observation bits if needed.
    bool invalidObs = false; // or decode obs if you know the meaning

    if (lowQuality || invalidObs) {
        // ignore this frame to avoid bad velocity
        return;
    }
...
//    sensor_flow[1]=filtered_raw_flow[1]*constrainf((opticflowHeight), height_min, height_max);
    ...
    // sensor_flow[0] = filtered_raw_flow[0] * constrainf((NewSensorRange_L1 * 0.001), height_min, height_max);
    // sensor_flow[1] = filtered_raw_flow[1] * constrainf((NewSensorRange_L1 * 0.001), height_min, height_max);
    
    ins_height = constrainf(ins_height, height_min, height_max);
    sensor_flow[0] = filtered_raw_flow[0] * ins_height;
    sensor_flow[1] = filtered_raw_flow[1] * ins_height;
    ...
    const float FLOW_DEADBAND = 0.01f;  // rad/s * m ~ m/s equivalent, tune

    if (fabsf(sensor_flow_hf[0]) < FLOW_DEADBAND) sensor_flow_hf[0] = 0.0f;
    if (fabsf(sensor_flow_hf[1]) < FLOW_DEADBAND) sensor_flow_hf[1] = 0.0f;
    ...
    if (ARMING_FLAG(ARMED)) {
        ...
        // Flow-based velocity
        VelocityX = sensor_flow_hf[0] * 100.0f;   // cm/s
        VelocityY = sensor_flow_hf[1] * 100.0f;
    
        // Integrate position, but with accel contribution *and* some damping
        PositionX += (VelocityX * dt) + (0.5f * accel_hf[0]) * (dt * dt);
        PositionY += (VelocityY * dt) + (0.5f * accel_hf[1]) * (dt * dt);
    
        // Simple velocity damping to reduce long-term drift
        const float VEL_DAMP = 0.98f;   // tune between 0.95–0.995
        VelocityX *= VEL_DAMP;
        VelocityY *= VEL_DAMP;
    ...
    velocity_hf[0] = sensor_flow_hf[0];
    velocity_hf[1] = sensor_flow_hf[1];
    Monitor_Println("GRAPH:raw_flow_x",  (double)raw_flow[0], 3);
    Monitor_Println("GRAPH:raw_flow_y",  (double)raw_flow[1],        3);
    Monitor_Println("GRAPH:sensor_flow_x", (double)sensor_flow_hf[0], 3);
    Monitor_Println("GRAPH:sensor_flow_y", (double)sensor_flow_hf[1], 3);
    Monitor_Println("GRAPH:of_height",   (double)opticflowHeight,    3);
